...
<%--            </c:if>--%>
            <c:choose>
                <c:when test="${requestScope.superuserSessione}">
                        ...
                        <%
                            // va bene o devo mettere plusDays(2)???
                             if((p.getDataInizio()).compareTo(LocalDate.now())>=0){
                                ...
                                if((MulteService.getMulteService().verificaUtenteMorosoByCF(p.getUtente().getCodiceFiscale()) && !(p.isApprovata())) || !(p.isApprovata())){%>
                                    <td>
